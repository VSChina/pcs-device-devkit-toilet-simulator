...
int DevKitMQTTClient_SendReportedStatus(STRING_HANDLE data)
{
    int result = 0;

    if (IoTHubClient_LL_SendReportedState(iotHubClientHandle, (const unsigned char*)STRING_c_str(data), STRING_length(data), NULL, NULL) != IOTHUB_CLIENT_OK)
    {
        result = __FAILURE__;
    }
    else
